...
def user_statistics_view(request):
    ...
    #             insert_data.save()
    #             print('年度qa已承接已更新')
    #         all_finished_date = feishu_project.get_all_user_finished_demand(create_date=None, uid=None,
    #                                                                         finished_time=20250101)
...
def get_all_user_msg(request):
    ...
    if request.method == 'GET':
        ...
        if all_date:
            ...
            # else:
            #     return JsonResponse({"status": False, "message": "没有数据"})
    return JsonResponse({"status": False, "message": "仅支持GET请求"})
...
def get_last_time(request):
    ...
    if query_time:
        datatime = str(query_time.datatime)
        print(datatime)
        return JsonResponse({"status": True, "data": datatime})
    return JsonResponse({"status": False, "message": "没有数据"})
