...
def generate_charts(bonus_list, free_game, cname, money, user_gold, user_last_gold):
    ...
    html_wrapped = "{% verbatim %}\n" + html_str + "\n{% endverbatim %}"
    with open(fr"D:\project\starx_project\interface_test\myapp\templates\{cname}.html", "w", encoding="utf-8") as f:
...
def candy_party_sugar(times, cname):
    ...
    headers = {
        ...
        'sec-fetch-mode': 'cors',
        'sec-fetch-dest': 'empty',
        'referer': 'https://m-test.starmakerstudios.com/mini-game/sugarRush/?promotion_id=6113&showBar=0&showNavigation=false&_sxiq_open_origin=banner_officialevent',
        'accept-language': 'zh-TW,zh-CN;q=0.9,zh;q=0.8,en-US;q=0.7,en;q=0.6',
        'priority': 'u=1, i'
    }
    b = 0
    ...
    for i in range(times):
        try:
            ...
            print(f'第{b}次执行异常{e},{response.text}')
            error.append(f'第{b}次执行异常{response.text}')
            continue
    response = requests.request("GET", url1, headers=headers)
...
def new_fish(times):
    ...
    for i in range(times):
        ...
        if response.json()['data']['code'] == 0:
            ...
            print(response.json())
    print(fish_list)
    print(f'钓鱼次数{a}，鱼的数量{len(fish_list)}, 鱼的价格{sum(prices)}')
...
if __name__ == '__main__':
    # new_fish(100)
    new_fish(10000)
    # candy_party_sugar(100000, 'candy_party_sugar')
